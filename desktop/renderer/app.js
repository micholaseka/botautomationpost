...
const els = {
    ...
    sku: document.getElementById("sku"), label: document.getElementById("label"), brand: document.getElementById("brand"),
    category: document.getElementById("category"), condition: document.getElementById("condition"),
    location: document.getElementById("location"), manualLocationStatus: document.getElementById("manualLocationStatus"),
    description: document.getElementById("description"), loops: document.getElementById("loops"),
    logContainer: document.getElementById("logContainer"),
    statusBadge: document.getElementById("statusBadge"), statusDot: document.getElementById("statusDot"),
    statusText: document.getElementById("statusText"), loginStatusBox: document.getElementById("loginStatusBox"),
    loginStatusLabel: document.getElementById("loginStatusLabel"), loginIcon: document.getElementById("loginIcon"),
    pairingDetail: document.getElementById("pairingDetail"), metricPhotos: document.getElementById("metricPhotos"),
    metricPosts: document.getElementById("metricPosts"), progressBar: document.getElementById("progressBar"),
...
function getLocations() {
    return els.location.value.split("/").map(value => value.trim()).filter(Boolean);
}

function getTotalPosts() {
    return getLocations().length * Math.max(1, Number.parseInt(els.loops.value, 10) || 1);
}
...
function addPhotos(files) {
    ...
    state.photos.push(...imageFiles.map(file => ({ file, url: URL.createObjectURL(file) })));
    renderPreviews();
    updatePairingInfo();
}
...
function updatePairingInfo() {
    ...
    if (!state.photos.length) {
        els.pairingDetail.textContent = "Belum ada foto. Upload foto untuk melihat skema pasangan.";
        return;
    }
    if (!locations.length) {
...
els.uploadArea.addEventListener("dragover", event => { event.preventDefault(); els.uploadArea.classList.add("dragover"); });
...
els.btnClearLog.addEventListener("click", () => {
    els.logContainer.innerHTML = '<div class="log-placeholder">Log dibersihkan.</div>';
});
...
function setPostingState(isPosting) {
    ...
    if (!isPosting) {
        ...
        els.manualLocationStatus.textContent = "Status lokasi: menunggu formulir dibuka.";
    ...
    els.btnContinuePost.classList.toggle("visible", isPosting && state.awaitingManualAction && state.requiresManualConfirmation);
    els.btnContinuePost.disabled = !state.awaitingManualAction || !state.requiresManualConfirmation;
...
function setLoginState(isLoggedIn) {
    ...
    els.loginStatusLabel.textContent = isLoggedIn ? "Status Login: Terdeteksi Login (Sesi Aktif)" : "Status Login: Belum Login";
    els.formLockBadge.innerHTML = isLoggedIn ? "&#x2705; Login Terdeteksi" : "&#x1F512; Login Diperlukan";
    els.formLockBadge.classList.toggle("unlocked", isLoggedIn);
    setPostingState(state.isPosting);
    if (isLoggedIn && !state.isPosting) setStatus("active", "Sudah Login · Siap Posting");
}
...
function validateListing() {
    ...
    if (!els.category.value) return "Pilih kategori Marketplace.";
    if (!getLocations().length) return "Isi minimal satu lokasi target. Pisahkan dengan tanda /.";
    return "";
}

function buildListing() {
    return {
        photos: state.photos.map(({ file }) => api.getFilePath?.(file) || file.path),
        title: els.title.value.trim(), price: els.price.value.trim(), sku: els.sku.value.trim(),
        label: els.label.value.trim(), brand: els.brand.value.trim(),
        category: els.category.value.trim(), condition: els.condition.value,
        description: els.description.value.trim(), locations: getLocations(),
...
function bridgeIpc() {
    if (!api) {
        appendLog("electronAPI tidak tersedia. Jalankan aplikasi melalui Electron.", "error");
        return;
    }
    ...
    api.onManualActionRequired(data => {
        ...
        if (data.waiting) {
            state.detectedLocation = "";
            els.manualLocationStatus.textContent = "Status lokasi: bot sudah mengetik lokasi; pilih saran lokasi dan Terbitkan di Marketplace.";
            els.manualLocationStatus.className = "manual-location-status";
        }
        setPostingState(state.isPosting);
        if (data.waiting) {
            appendLog(`${data.tag} menunggu Anda memilih lokasi dan menerbitkan listing di tab Marketplace. Tab berikutnya akan terbuka otomatis setelah terdeteksi.`, "warn");
        }
    });
    api.onManualLocationSelected(data => {
        ...
        appendLog(`${data.tag} lokasi manual terdeteksi: ${data.selectedLocation}.`, data.matchesExpected ? "success" : "warn");
    });
}
...
els.btnOpenBrowser.addEventListener("click", async () => {
    ...
    try {
        ...
        if (!result.success) throw new Error(result.error || "Browser tidak dapat dibuka.");
        els.btnCloseBrowser.disabled = false;
        appendLog("Browser terbuka. Selesaikan login Facebook secara manual.", "warn");
...
els.btnCloseBrowser.addEventListener("click", async () => {
    ...
    try {
        ...
        if (!result.success) throw new Error(result.error || "Browser tidak dapat ditutup.");
    } catch (error) {
...
els.btnPost.addEventListener("click", async () => {
    ...
    if (validationMessage) { appendLog(validationMessage, "error"); return; }
    if (!api) { appendLog("electronAPI tidak tersedia.", "error"); return; }
    ...
    appendLog(`Memulai batch: ${state.totalPosts} formulir, ${listing.photos.length} foto.`, "info");
    try {
        const result = await api.startPost(listing);
        if (!result.success && !result.cancelled) throw new Error(result.error || "Batch gagal dimulai.");
    } catch (error) {
        setPostingState(false);
...
els.btnCancelPost.addEventListener("click", async () => {
    ...
    try {
        ...
        if (!result.success) throw new Error(result.error || "Batch tidak dapat dibatalkan.");
        appendLog("Permintaan pembatalan dikirim. Menyelesaikan tahap aktif…", "warn");
    } catch (error) {
...
els.btnContinuePost.addEventListener("click", async () => {
    ...
    try {
        ...
        if (!result.success || !result.continued) {
            throw new Error(result.error || "Tidak ada formulir yang menunggu konfirmasi.");
        }
        state.awaitingManualAction = false;
        setPostingState(true);
        appendLog("Penerbitan dikonfirmasi. Bot akan membuka formulir baru bila masih ada sisa batch.", "success");
    } catch (error) {
...
(async () => {
    ...
    } catch { /* Initial status is already rendered. */ }
})();
